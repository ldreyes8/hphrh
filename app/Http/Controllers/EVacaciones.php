...
class EVacaciones extends Controller
{
  ...
  public function calculardias(request $request)
  {
    ...
    else
    {
  	...
  	public function diashatomar(request $request)
  	{
		...
		//Solo aplica para el año 2017
	    $navidad = '25/12/2017';
	    $navidad = Carbon::createFromFormat('d/m/Y',$navidad);

	    $añonuevo = '01/01/2018';
	   	$añonuevo = Carbon::createFromFormat('d/m/Y',$añonuevo);
	      	...
	      	if($emergencia === "1")                               		//Se valida si esta activo el check de emergencia
	      	{
	        	while ($ffin >= $fini) {                        		//Mientras $fecha final sea mayor o igual a fecha inicio
	          		if($fini <= $ffin){                               	//Se valida que fecha inicio no sea igual a fecha final
	            		...
	            		if($fini == $navidad){
	            			if($navidad->isWeekend() === false){        //Se valida que fecha inicio no sea un fin de semana
	              				$days--;								//Se suma dias a tomar
	            			}
	            		}

	            		if($fini == $añonuevo){
	            			if($añonuevo->isWeekend() === false){       //Se valida que fecha inicio no sea un fin de semana
	              				$days--;								//Se suma dias a tomar
	            			}
	            		}												//Se cierra el if

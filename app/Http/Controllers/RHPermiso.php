...
class RHPermiso extends Controller
{
    ...
    public function indexconfirmado (Request $request)
    {
       $permisos = DB::table('asignajefe as aj')
	        ...
	        ->join('persona as per','emp.identificacion','=','per.identificacion')
	        ->join('tipoausencia as tp','au.idtipoausencia','=','tp.idtipoausencia')
	        ->select(DB::raw('CONCAT(per.nombre1," ",per.apellido1," ") AS nombre'),'per.identificacion','au.fechasolicitud','tp.ausencia','au.fechainicio','au.fechafin','au.idausencia','au.totaldias','au.totalhoras','au.justificacion','au.observaciones')
